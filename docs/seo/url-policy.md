...
| INT-01–07 | `scripts/intelligence.mjs`, `scripts/articles.mjs`, `scripts/editorial.mjs`, `content/briefs.json`, and `content/articles/metadata.json` own routes and publication eligibility. Pages are anonymous static GETs. `publicationStatus` plus approval gates distinguish published records from drafts. All generated HTML templates declare the preferred origin; PDFs are downloads and presently lack the separate HTTP canonical lifecycle required by INT-06. |
