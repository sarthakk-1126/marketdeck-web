...
import {catalog,shelf,hub,libraryPage,archivePage,learningHub,LEARNING_HUBS,atomFeed} from './intelligence.mjs';
...
export function buildEditorial({data,template,root,review}) {
  ...
  const feedPath=resolve(root,'intelligence/feed.xml');
  mkdirSync(dirname(feedPath),{recursive:true});
  writeFileSync(feedPath,atomFeed(collection,{origin:data.brand.url}));
  ...
  template=template.replace('</head>','<link rel="alternate" type="application/atom+xml" title="MarketDeck Intelligence" href="/intelligence/feed.xml"><link rel="stylesheet" href="/intelligence-shelf-v2.css"><script src="/intelligence-shelf-v2.js" defer></script></head>');
