...
const productOverview = data.products.map((p, i) => `<article class="suite-overview-card ${p.color}" data-overview-product="${p.id}"><div class="suite-overview-card-top"><span class="suite-overview-icon">${icon(p.icon)}</span><span class="suite-overview-number">0${i + 1}</span></div><p class="suite-overview-category">${escapeHtml(p.category)}</p><h3>${escapeHtml(p.name)}</h3><p class="suite-overview-lead">${escapeHtml(p.headline)}</p><p class="suite-overview-capabilities">${p.features.map(escapeHtml).join(' · ')}</p>${destination(p.url, p.action, 'suite-overview-link', 'Destination link pending')}</article>`).join('');
const communityTabs='';
