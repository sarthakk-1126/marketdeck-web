...
const browser=await chromium.launch({headless:true});const report={origin,pages:[],accessibility:[],noJS:[],links:[]};
try {
 for(const path of paths){
  for(const width of [320,390,768,1440,1920]){
   ...
   if(width===390||width===1440){
    ...
    const a=await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21a','wcag21aa']).analyze();report.accessibility.push({path,width,violations:a.violations});assert.equal(a.violations.length,0,JSON.stringify(a.violations));
   ...
   report.pages.push({path,width,status:response.status(),overflow:checks.overflow,canonical:checks.canonical});await page.close();
  }
 }
